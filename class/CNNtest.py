...
train_data_dir = 'D:/施雨欣/深度学习/Test/mix_t-2/'  # 训练集数据目录路径
val_data_dir = 'D:/施雨欣/深度学习/Test/mix_v-2/'  # 验证集数据目录路径
...
elabel=val_labels
# 划分训练集和验证集
train_images, test_images, train_labels, test_labels = train_test_split(train_images, train_labels, test_size=0.4, stratify=train_labels, random_state=42)
...
for train_index, val_index in kf.split(train_images):
    ...
    # print(len(train_images))
    # # 训练集和验证集划分
    # train_x_fold, val_x_fold = train_images[train_index], train_images[val_index]
    # train_y_fold, val_y_fold = train_labels[train_index], train_labels[val_index]
    train_images, test_images, train_labels, test_labels = train_test_split(train_images, train_labels, test_size=0.2,
                                                                            stratify=train_labels, random_state=42)
    print(test_labels)
    train_generator = generate_arrays_from_file(train_images, train_labels, batch_size=9)
    validation_generator = generate_arrays_from_file(val_images, val_labels, batch_size=5)

    # 将训练集和验证集传入生成器
    # train_generator = generate_arrays_from_file(x_train, y_train,12)
    # val_generator = generate_arrays_from_file(x_val, val_y_fold,10)
    ...
    # 评估模型
    loss, accuracy = model.evaluate(generate_arrays_from_file(test_images, test_labels,6), verbose=1, steps=1)

    # # 模型评估
    # predictions = model.predict_generator(generate_arrays_from_file(edata,elabel, 12), steps=1)
    #
    # # 计算准确率和混淆矩阵
...
# 最后，在测试集上评估模型
loss, accuracy = model.evaluate(generate_arrays_from_file(edata, elabel,6), verbose=1, steps=1)
print(loss,accuracy)
# 生成预测结果
...
predictions = model.predict(generate_arrays_from_file(edata,elabel, 6), steps=1)
