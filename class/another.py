...
# print(len(train_x))
def generate_arrays_from_file(date_x, label_y, batch_size=7):
    count = 0
    while True:
...
class SaveModelCallback(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_accuracy is not None and val_accuracy >= 0.75:
            self.model.save('bzmodel_at_epoch_{}_val_acc_{:.4f}.h5'.format(epoch+1, val_accuracy))  # 保存模型
...
#     generate_arrays_from_file(train_x, train_y),
#     shuffle=True,
#     steps_per_epoch=16,
#     epochs=75,
#     validation_data=generate_arrays_from_file(test_x, test_y),
#     validation_steps=1,
#     callbacks=[SaveModelCallback(),reduce_lr],
# )
# filepath = 'test-model.h5'
# filepath = 'model_at_epoch_272_val_acc_0.8571.h5'
# filepath = "D:/施雨欣/深度学习/分类器/0/0.57/model_at_epoch_285_val_acc_0.9375.h5"#0.58
# filepath = "1/0.5314/model_at_epoch_206_val_acc_0.8571.h5"#0.58/0.60
filepath = "1/0.5314/model_at_epoch_199_val_acc_0.8000.h5"
print(filepath)
model = keras.models.load_model(filepath)
loss, accuracy = model.evaluate(generate_arrays_from_file(test_x, test_y), verbose=1, steps=25)
print(loss, accuracy)
...
predictions = model.predict(generate_arrays_from_file(test_x, test_y), steps=25, verbose=1)
# print(predictions)
