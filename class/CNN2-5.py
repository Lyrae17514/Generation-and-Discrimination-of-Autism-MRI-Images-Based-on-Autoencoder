...
# print(len(train_x))
def generate_arrays_from_file(date_x, label_y, batch_size=12):
    count = 0
    while True:
...
# loss,accuracy = model.evaluate(generate_arrays_from_file(T1,5,False), verbose=1, steps=8)
loss,accuracy = model.evaluate(generate_arrays_from_file(test_x, test_y,12), verbose=1, steps=1)
print(loss,accuracy)
# 生成预测结果
predictions = model.predict(generate_arrays_from_file(test_x, test_y,12), steps=1)
