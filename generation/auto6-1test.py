...
path_G='D:/施雨欣/深度学习/自闭症/871-control-T1/img_1/'
save_path = 'D:/施雨欣/深度学习/Results/871/control/img128/'
...
def randchoose():
    while 1:
        ...
        cc = T1_file_name[random.randint(0, 1871)]
        # cc = T1_file_name[0]
        print(cc)
...
def svae_image(generator,num):
    ...
    z = np.random.normal(0, 1, (1, 16384))
    gen_imgs = generator.predict(z)
    # gen_imgs = np.array(gen_imgs).reshape([8000000]).reshape((200,200,200))
    gen_imgs = np.array(gen_imgs).reshape([16384]).reshape((128, 128))
    OrthoSlicer3D(gen_imgs).show()
    # print(gen_imgs.shape)
    new_image = nib.Nifti1Image(gen_imgs, np.eye(4))
    nib.save(new_image, r'D:/施雨欣/深度学习/Results/871/control/img128/' + strtime + '.png')
...
decoded = Dense(units=16384, activation='tanh')(decoded)
