...
path_G='D:/施雨欣/深度学习/自闭症/MaxMun_t1/control/'
save_path = 'D:/施雨欣/深度学习/Results/Max/autoencoder_nii/'
...
def randchoose():
    while 1:
        ...
        cc = T1_file_name[random.randint(0, 32)]
        # cc = T1_file_name[0]
        print(cc)
...
def svae_image(generator,num):
    ...
    z = np.random.normal(0, 1, (1, 262144))
    gen_imgs = generator.predict(z)
    # gen_imgs = np.array(gen_imgs).reshape([8000000]).reshape((200,200,200))
    gen_imgs = np.array(gen_imgs).reshape([262144]).reshape((64, 64, 64))
    OrthoSlicer3D(gen_imgs).show()
    # print(gen_imgs.shape)
    new_image = nib.Nifti1Image(gen_imgs, np.eye(4))
    nib.save(new_image, r'D:/施雨欣/深度学习/Results/Max/autoencoder_nii/con_64/' + strtime + '.nii.gz')
...
decoded = Dense(units=262144, activation='tanh')(decoded)
...
plt.savefig("Loss_{:d}e.jpg".format(epochs))
# 保存生成器生成的图像
